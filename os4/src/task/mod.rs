    ...
    fn run_next_task(&self) {
        if let Some(next) = self.find_next_task() {
            ...
            if inner.tasks[next].time == 0 {
                inner.tasks[next].time = get_time_us() / 1000;
            }
...
pub fn update_ti(syscall_id: usize) {
    TASK_MANAGER.update_ti(syscall_id);
}

pub fn get_ti() -> TaskInfo {
    TASK_MANAGER.get_ti()
}
